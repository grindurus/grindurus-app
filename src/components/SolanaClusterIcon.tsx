...
type SolanaClusterId = 'mainnet-beta' | 'devnet'

type Props = {
  clusterId: SolanaClusterId
  /** Outer box size in px (default 20, matches other network icons). */
  size?: number
  className?: string
}

/** Solana mainnet = brand logomark. Devnet = purple testnet badge (like Sepolia vs ETH). */
export function SolanaClusterIcon({ clusterId, size = 20, className }: Props) {
  const reactId = useId().replace(/:/g, '')

  if (clusterId === 'mainnet-beta') {
    return (
      <span className={className} style={{ display: 'inline-flex', lineHeight: 0 }}>
        <SolanaLogomark size={Math.round(size * 0.8)} title="Solana Mainnet" />
      </span>
    )
  }

  const bgId = `sol-dev-bg-${reactId}`
  return (
    <svg
      width={size}
      height={size}
