...
/** Compact network picker for GRS CA — lists configured deployments, works without a connected wallet. */
export function GrsCaNetworkSelect({ config, ariaLabel = 'Select GRS contract network' }: Props) {
  ...
  const selected = useMemo(() => {
    if (config?.kind === 'solana') {
      return options.find((item) => item.kind === 'solana' && item.cluster === config.cluster) ?? null
    }
    if (config?.kind === 'evm') {
      return options.find((item) => item.kind === 'evm' && item.chainId === config.chainId) ?? null
    }
