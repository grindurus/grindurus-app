import { useCallback } from 'react'
import { isAddress, type Hex } from 'viem'
import { executeEvmGrindersMint } from '../grai/evm/executeTransactions'
import { useGraiDeployment } from '../grai/GraiDeploymentProvider'
import { useGraiEvmTransaction } from './useGraiEvmTransaction'
import type { GraiTransactionStatus } from './useGraiTransaction'

export type GraiGrindersMintStatus = GraiTransactionStatus

const ZERO_ADDRESS = '0x0000000000000000000000000000000000000000' as const

function parseAddress(input: string, label: string, allowZero = false): `0x${string}` {
  const trimmed = input.trim()
  if (!trimmed) {
    if (allowZero) return ZERO_ADDRESS
    throw new Error(`Enter ${label}`)
  }
  if (!isAddress(trimmed)) {
    throw new Error(`${label} must be a valid address`)
  }
  return trimmed as `0x${string}`
}

export function useGraiGrindersMint() {
  const { chainKind, evm } = useGraiDeployment()
  const { run, reset, status, error, lastHash, isPending } = useGraiEvmTransaction()

  const mint = useCallback(
    async (params: {
      custodianKind: Hex
      ownerInput: string
      baseAssetInput: string
      quoteAssetInput: string
    }) => {
      if (chainKind !== 'evm') {
        throw new Error('Grinders.mint is only available on EVM')
      }
      if (!evm) throw new Error('GRAI is not configured for this EVM network')

      const owner = parseAddress(params.ownerInput, 'owner', true)
      const baseAsset = parseAddress(params.baseAssetInput, 'base asset')
      const quoteAsset = parseAddress(params.quoteAssetInput, 'quote asset')

      if (baseAsset.toLowerCase() === quoteAsset.toLowerCase()) {
        throw new Error('Base and quote assets must differ')
      }

      const { hash } = await run({
        connectMessage: 'Connect the Grinders owner wallet to mint a custodian',
        chainAction: 'mint custodian',
        failureMessage: 'Grinders mint transaction failed',
        execute: () =>
          executeEvmGrindersMint({
            config: evm,
            custodianKind: params.custodianKind,
            owner,
            baseAsset,
            quoteAsset,
          }),
      })
      return hash
    },
    [chainKind, evm, run],
  )

  return {
    mint,
    reset,
    status,
    error,
    lastSignature: lastHash,
    isMinting: isPending,
  }
}
